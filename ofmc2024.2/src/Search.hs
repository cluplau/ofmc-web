   ...
   in if attack_states == []
        then Nothing
          ...
          let attack_state --- @(b,t,f,oi,ni,po,cs,h)
           ...
           in Just (prlist (show errmsg) attack_state)
...
wrids2 n p (Node a l) =
  case p a of
    ...
    Nothing -> (rest, nds)
  where
    (rest, nds) = foldr go ("", 0) (map (wrids2 (n - 1) p) l)
    go (s1, i1) (s2, i2) = case s1 of
      '*' : _ -> (s1, i1)
      _ -> (s2, i1 + i2)

-- all in: report ALL attack states, except descendants of an attack state.
allin :: Int -> (State -> Maybe String) -> Tree State -> [String]
allin 0 p (Node a l) = case p a of
  Just att -> [att]
  Nothing -> []
allin n p (Node a l) =
  ( case p a of
    ...
    ++ if (n > 0) then concatMap (allin (n - 1) p) l else []
...
neusuch theo must por rules hcs p balance depth sfb (xs', ys) (s1, s2) =
  let xs = take balance xs'
      results = catMaybes $ map (p . snd) xs
      successors =
        if must
          then error "MUST CURRENTLY not supported"
          else
...
exec_check0 theo por rules p balance depth sfb (x : xs, ys) stats =
  let isAttack = (p (snd x))
   in if (isJust isAttack)
        then (isAttack, stats)
        else
          let succ_x = (successor theo por sfb rules x)
                  ...
                  if (((length succ_x) < balance))
                    then
                      exec_check0
                        ...
                        rules
                        p
                        (balance - (length succ_x) + 1)
                        depth
                        sfb
                        (xs, (reverse succ_x) ++ ys)
                        (inc_vip stats)
                    else
        ...
        ( map
            ( \(c, session_facts) ->
                exec_check0
                  ...
                  por
                  rules
                  (\state -> check_states state final_states)
                  100000
                  100000
                  100000
                  ( [ ( a,
            ...
            (zip (map ((*) (length rules)) [1 .. n]) state_partition)
        )
  ...
  let showalt [] i = ""
      showalt (x : xs) i =
        indent
          ...
          ++ (show i)
          ++ ") "
          ++ (nprinttl2 (history state) x)
          ++ (printniveau2 (indent ++ "|") x)
          ++ (showalt xs (i + 1))
   in showalt l 0
