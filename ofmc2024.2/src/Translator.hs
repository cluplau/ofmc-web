...
--- Checking Compliance for Capture the Flag -- given submitted
--- protocol and the template protocol that specifies the challenge

check_compliance :: Protocol -> Protocol -> Bool
check_compliance (_,types,knowledge,[],actions,goals) (_,typesS,knowledgeS,[],_,goalsS) =
  -- the above pattern match already ignores the names given to the
  -- protocol, demands no abstractions are used, and ignores the actions of the specification
  -- if (types/=typesS) then 
  if (not $ check_types_compliance types typesS []) then error "You have changed the type specification" else
  if (knowledge/=knowledgeS) then error "You have changed the knowledge specification" else
  if (goals/=goalsS) then error "You have changed the goals" else
  all check_action actions

check_action :: Action -> Bool
check_action (((_,False,Nothing),Insecure,(_,False,Nothing)),_,Nothing,Nothing) = True
check_action _ = error "CTF compliance: using channels is not allowed."

check_types_compliance :: [(Type,[Ident])] -> [(Type,[Ident])] -> [Ident] -> Bool
check_types_compliance types typesS declared =
  case typesS of
  [] -> case types of
        [] -> True
        ((t,ids):tids) ->
          case t of
          Agent _ _ -> error $ "CTF compliance: you cannot introduce new agents! You added "++show ids++" where the original spec does have no more identifiers."
          _ -> if any (`elem` declared) ids then error "CTF compliance: you cannot re-declare identifiers!"
               else check_types_compliance tids typesS $ ids++declared
  ((tS,idsS):tidsS) ->
     case types of
     [] -> error "CTF compliance: some type specificiations were dropped."
     ((t,ids):tids) ->
       if (t/=tS) then
          case t of
          Agent _ _ -> error $ "CTF compliance: you cannot introduce new agents! You declared a new set of agents "++show ids
          _ -> if any (`elem` declared) ids then error "CTF compliance: you cannot re-declare identifiers!"
               else check_types_compliance tids typesS $ ids++declared
       else
           case t of
           Agent _ _ -> if (ids==idsS) then check_types_compliance tids tidsS $ ids++declared
                        else error $ "CTF compliance: you cannot introduce new agents!"++show ids
           _ -> if isSubsequenceOf idsS ids then check_types_compliance tids tidsS $ ids++declared
                else error "CTF compliance: some type specificiations were dropped."
...
fKnow formats (typings,diseqs) = ([(a,nub $ Atom a : chanKnow a ++ (map Atom formats) ++ map (fMsg formats) ms ) | (a,ms)<-typings],
                                  [(fMsg formats s, fMsg formats t) | (s,t)<-diseqs ])
  where chanKnow a = [Comp Pseudonym [Atom a],
...
fAct formats ((sp@(sender,b,Nothing),Authentic,rp@(receiver,False,Nothing)),m,Nothing,Nothing) =
  let senderpk = if b then Comp Pseudonym [Atom sender] else Comp AuthChan [Atom sender]
      m'= if b then Comp Cat [senderpk,Comp Crypt [Comp Inv [senderpk],m]]
...
fAct formats ((sp@(sender,b1,Nothing),Secure,rp@(receiver,b2,Nothing)),m,Nothing,Nothing) =
  if b1 && b2 then error "Both endpoints of a secure channel pseudonymous. Why?\n" else
  let senderpk = if b1 then Comp Pseudonym [Atom sender] else Comp AuthChan [Atom sender]
      ...
      m' = if b1 then Comp Cat [senderpk, Comp Crypt [receiverpk, Comp Crypt [Comp Inv [senderpk],m]]]
...
{-
chtrafo _ (_,True,_) _ (_,True,_) msg =
  error "Not permitted currently: both ends of a transmission pseudonymous."

chtrafo isSender (sp@(sender,True,Just nym)) ct (rp@(receiver,False,_)) msg =
  case ct of
    Insecure -> error "Pseudonymous sending on an insecure channel is not meaningful."
    Confidential -> error "Pseudonymous sending on confidential channel is not meaningful."
    Authentic ->
      (Comp Cat 
	 [nym,
	  Comp Crypt 
    ...
    Secure ->
	 ...
	 [Atom "secChCr",
	  ...
	  [nym,
	   Atom receiver]],
	 msg]]
chtrafo isSender (sp@(sender,False,_)) ct (rp@(receiver,True,Just nym)) msg =
  case ct of
    Insecure -> error "Pseudonymous receiving on an insecure channel is not meaningful."
    Authentic -> error "Pseudonymous receiving on an authentic channel is not meaningful."
    Confidential -> (Comp Crypt [Comp Apply [Atom "confChCr",nym],msg]) 
    Secure ->
     Comp Scrypt 
        [Comp Apply 
	 [Atom "secChCr",
      ...
      folder (ChGoal ((speer@(sender,_,_)), channeltype, (rpeer@(receiver,_,_))) msg) rs =
	      ...
	      || channeltype==FreshAuthentic || channeltype==FreshSecure
	       then [Fact "witness" 
		     [goalpeer True -- peerToMsgKnwn 
		      ...
		      rpeer (head post),
		      mkPurpose (Comp Cat [nugoalpeer rpeer (head post2),
                                           nugoalpeer speer (head post2),
                                           msg]),pattern]]
               else [])
	    ...
	    newfacts' =
	      (if channeltype==FreshSecure || channeltype==FreshAuthentic
	       then [Fact "request" [nugoalpeer rpeer (head post2),
				     nugoalpeer speer (head post2),
				     mkPurpose (Comp Cat [nugoalpeer rpeer (head post2),
                                                          nugoalpeer speer (head post2),
                                                          msg]),pat,Atom "SID"]]
	       ...
	       if channeltype==Secure || channeltype==Authentic
	       then [Fact "wrequest" [nugoalpeer rpeer (head post2),
				      nugoalpeer speer (head post2),
				      mkPurpose (Comp Cat [nugoalpeer rpeer (head post2),
                                                           nugoalpeer speer (head post2),
                                                           msg]),pat]]
...
nugoalpeer :: Peer -> Rule -> Msg
nugoalpeer (a,False,Nothing) (_,[],_,r) = Atom a
{-
nugoalpeer isKnown (a,True,Nothing) _ = 
	 if isKnown then (Comp Apply [Atom "pseudonym",Atom a])
	 else Atom ("Pseudonym"++a)
-}
nugoalpeer (a,True,Nothing) (_,_,_,r) = 
  let msg = Comp Pseudonym [Atom a] in
  case head $ filter isState r of 
  (State role msgs) ->
    if msg `elem` (map snd msgs) then msg
    else let msg' = mkname msg
         in if msg' `elem` (map snd msgs) then msg'
            else error "Pseudo not found!"
  _ -> error $ "Function nugoalpeer receives rule with unexpected shape: "++(show r)
  where isState (State _ _ ) = True
        isState _ = False
...
---    (error $ (show aglili) ++ (show (concatMap (\ (x,y) -> " & "++(ppId x)++"/="++(ppId y)) [(x,y) | l<-aglili, x<-l, y<-l, x/=y])))++ 
    (concatMap (\ (s,t) -> if isVar s then " & "++(ppMsg IF s)++"/="++(ppMsg IF t)
                           else error $ "error with "++(ppMsg IF s)++"/="++(ppMsg IF t)++": first element in a disequation needs to be a variable ")) ineq'++
    "\n\n"++
    "section rules:\n"}
    where isVar (Atom x) = isVariable x
          isVar _ = False
